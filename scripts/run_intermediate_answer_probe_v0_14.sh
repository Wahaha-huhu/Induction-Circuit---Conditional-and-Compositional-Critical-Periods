...
OUT_DIR=${OUT_DIR:-${RUNS_DIR}/intermediate_answer_probe}
DEVICE=${DEVICE:-cuda}
...
SELECTED_TXT="$OUT_DIR/probe_selected_runs.txt"
SELECTED_JSONL="$OUT_DIR/probe_selected_runs.jsonl"
...
while IFS= read -r rec; do
  ...
  n=$((n+1))
  out="$OUT_DIR/per_run/${group}_${arm}_seed${seed}_intermediate_answer_probe.csv"
  echo "[intermediate-answer probe] $n group=$group arm=$arm seed=$seed $run_dir"
  extra=()
  if [[ "$SKIP_LINEAR_PROBE" == "1" ]]; then
    extra+=(--skip-linear-probe)
  fi
  python scripts/run_intermediate_answer_probe.py "$run_dir" \
    --device "$DEVICE" \
...
python scripts/aggregate_intermediate_answer_probe.py \
  --runs-dir "$OUT_DIR" \
  --out-dir "$OUT_DIR"

python scripts/pack_results.py \
  --runs-dir "$OUT_DIR" \
  --out cp_toy_intermediate_answer_probe_v0_14.zip
